...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="cubic10D")
    ...
    if configs['data_name'] == 'cubic10D':

        cubic10D_results_path = './cubic10D_results/'

        ## generate cubic 10D data
        Gen = CL_cubic_10D()
        ...
        xtrain = (torch.rand(Ntrain, Npar)*4.0-2.0).to(device)
        ytrain = Gen.target_fun(xtrain).to(device)
        ...
        Nvalid = 1000
        xvalid = (torch.rand(Nvalid, Npar) + 2.0).to(device)
        yvalid = Gen.target_fun(xvalid).to(device)
    ...
    xTrain = xtrain_normal
    yTrain = ytrain_normal
    xTest = xvalid_normal
    yTest = yvalid_normal
    ...
    for network_name in ["ConditionalQuantile"]:
        ...
        ### PIW analysis
        train_PIW_arr = (p_high_tr - p_low_tr).detach().numpy()
        test_PIW_arr = (p_high_te - p_low_te).detach().numpy()

        np.savetxt('SQR_MPIW_train_bias.dat', train_PIW_arr)
        np.savetxt('SQR_MPIW_test_bias.dat', test_PIW_arr)

        kde_train = stats.gaussian_kde(train_PIW_arr.flatten())
        kde_test = stats.gaussian_kde(test_PIW_arr.flatten())
        ...
        plt.legend()
        plt.savefig('SQR_cubic10D_bias.png')
        plt.show()


        ###------- Option I: calculate confidence interval
        conf_score = kde_train(test_PIW_arr.flatten())/p1.max()
