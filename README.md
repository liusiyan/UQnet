...
We use a one-dimensional non-Gaussian cubic regression dataset to illustrate PI3NN. We train models on $y = x^3 + \varepsilon$ within $[-4,4]$ and test them on $[-7,7]$. The noise $\varepsilon$ is defined by $\varepsilon = s(\zeta)\zeta$, where $\zeta \sim \mathcal{N}(0,1)$, $s(\zeta) = 30$ for $\zeta \ge 0$ and $s(\zeta) = 10$ for $\zeta < 0$.
