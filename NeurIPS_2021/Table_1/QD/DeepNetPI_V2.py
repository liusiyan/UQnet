...
class TfNetwork:
	def __init__(self, x_size, y_size, h_size, type_in="pred_intervals", 
		...
		soften=100., lambda_in=10., sigma_in=0.5, activation='relu', bias_rand=False, out_biases=[2.,-2.], rnd_seed=None,
		**kwargs):
		"""
		...
		tf.random.set_random_seed(self.rnd_seed)
	...
	def train(self, sess, X_train, y_train, X_val, y_val, n_epoch, l_rate=0.01,
		...
		is_batch=False, n_batch=100, is_run_test=False, is_print_info=True,
	...
	def vis_train_v2(self, data_name=None, split_seed=None, rnd_seed=None, ensemble=None,
					 save_graphs=False, is_use_val=False, bool_save_loss=False,
					 save_loss_path='./Results/loss_history/',
					 plot_loss_path='./Results/loss_curves/'):
		"""
		view graph of training history
		"""
		...
		ax.set_xlabel("epochs")
		ax.set_ylabel("loss")
		suptitle = 'data: ' + data_name + ' splitting seed: ' + str(split_seed) + ' random seed: ' + str(rnd_seed) + ' ensemble: ' + str(ensemble)
		# ax.plot(NN.loss_log[:,0],NN.loss_log[:,2]) # val
		title = suptitle \
				+'\nloss=' + str(round(self.last_loss_trn, 4)) \
				+ ', loss_type=' + self.loss_type + ',' \
				+ '\nh_size=' + str(self.h_size) \
				+ ', soften=' + str(self.soften) \
				+ ', lambda_in=' + str(self.lambda_in)
		...
		fig.tight_layout()
		if save_graphs:
			fig.savefig(plot_loss_path+data_name+'_seed_'+str(split_seed)+'_'+str(rnd_seed)+'_ensemble_'+str(ensemble)+'.png')
			# fig.savefig('02_outputs/fig_grad_train.png', bbox_inches='tight')

		if bool_save_loss:
			''' Save the loss data for further analysis '''
			df_loss = pd.DataFrame({
				'iter': self.loss_log[:, 0],
				'train_loss': self.loss_log[:, 1],
				'test_loss': self.loss_log[:, 2]
			})
			df_loss.to_csv(
				save_loss_path + data_name + '_loss_seed_' + str(split_seed) + '_' + str(rnd_seed) + '_ensemble_' + str(ensemble) + '.csv')
