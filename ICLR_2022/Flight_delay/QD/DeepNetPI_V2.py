...
class TfNetwork:
	...
	def vis_train_v2(self, data_name=None, rnd_seed=None, ensemble=None,
		...
		ax.set_xlabel("epochs")
		ax.set_ylabel("loss")
		suptitle = 'data: ' + data_name  + ' random seed: ' + str(rnd_seed) + ' ensemble: ' + str(ensemble)
		# ax.plot(NN.loss_log[:,0],NN.loss_log[:,2]) # val
		title = suptitle \
		...
		fig.tight_layout()
		if save_graphs:
			fig.savefig(plot_loss_path+data_name+'_seed_'+'_'+str(rnd_seed)+'_ensemble_'+str(ensemble)+'.svg')
			# fig.savefig('02_outputs/fig_grad_train.png', bbox_inches='tight')
		...
		if bool_save_loss:
			...
			df_loss.to_csv(
				save_loss_path + data_name + '_loss_seed_' + '_' + str(rnd_seed) + '_ensemble_' + str(ensemble) + '.csv')
