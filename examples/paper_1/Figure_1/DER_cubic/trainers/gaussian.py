...
class Gaussian:
    ...
    def train(self, x_train, y_train, x_test, y_test, x_valid, y_valid, y_scale, batch_size=128, iters=10000, verbose=True):
        ...
        self.plot_scatter_with_var_from_pandas(x_train, y_train, x_valid, y_valid,
                                               load_test_df['test_mu'].values, load_test_df['test_var'].values, path=self.custom_plot_folder+"/test_plot.pdf", n_stds=3, test_bounds=valid_bounds, show=True)
        self.plot_scatter_with_var_from_pandas(x_train, y_train, x_valid, y_valid,
                                               load_valid_df['valid_mu'].values, load_test_df['test_var'].values, path=self.custom_plot_folder+"/valid_plot.pdf", n_stds=3, test_bounds=valid_bounds, show=True)
