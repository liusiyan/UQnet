...
class Dropout:
    ...
    def evaluate(self, x, y):
        ...
        if self.mse:
            ...
            # print(tf.shape(mean_mu))
            # print(tf.shape(mean_sigma))
    ...
    @tf.function
    def siyan_dropout_evaluate(self, x_input):
        # x_input = tf.convert_to_tensor(x, tf.float32)
        preds = tf.stack([self.model(x_input, training=True) for _ in range(5)], axis=0) #forward pass
    ...
    def train(self, x_train, y_train, x_test, y_test, x_valid, y_valid, y_scale, batch_size=128, iters=10000, verbose=True):
            ...
            if self.iter % 10 == 0:
                ...
                if test_eval_count == 0:
                    ...
                    if test_vloss < tmp_test_loss:
                        ...
                        # print(tf.shape(test_var))
                        # print(tf.shape(test_mu))
                        ...
                        # test_mean_mu = tf.reduce_mean(test_mu, axis=0)
                        # test_mean_var = tf.reduce_mean(test_var, axis=0)
                        # test_reduce_std_mu = tf.math.reduce_std(test_mu, axis=0)
                        # test_reduce_mean_var = tf.reduce_mean(test_var**0.5, axis=0)
                        #
                        # print(tf.shape(test_mu))
                        # print(tf.shape(test_mean_mu))
                        # print(tf.shape(test_mean_var))
                        # print(tf.shape(test_reduce_std_mu))
                        # print(tf.shape(test_reduce_mean_var))

                        test_mean_mu, test_epistemic = self.siyan_dropout_evaluate(x_test_input)
                        # (1000, 2)
                        test_results_df = pd.DataFrame({
                            ...
                            # 'test_mean_var': list(test_mean_var.numpy().flatten()),
                            # 'test_reduce_std_mu': list(test_reduce_std_mu.numpy().flatten()),
                            # 'test_reduce_mean_var': list(test_reduce_mean_var.numpy().flatten())
                        })
                ...
                if valid_eval_count == 0:
                    ...
                    if valid_vloss < tmp_valid_loss:
                        ...
                        # valid_mean_mu = tf.reduce_mean(valid_mu, axis=0)
                        # valid_mean_var = tf.reduce_mean(valid_var, axis=0)
                        # valid_reduce_std_mu = tf.math.reduce_std(valid_mu, axis=0)
                        # valid_reduce_mean_var = tf.reduce_mean(valid_var**0.5, axis=0)

                        valid_mean_mu, valid_epistemic = self.siyan_dropout_evaluate(x_valid_input)
                        ### update the DataFrame
        ...
        # epistemic_test = load_test_df['test_reduce_std_mu'].values + load_test_df['test_reduce_mean_var'].values
        # epistemic_valid = load_valid_df['valid_reduce_std_mu'].values + load_valid_df['valid_reduce_mean_var'].values
