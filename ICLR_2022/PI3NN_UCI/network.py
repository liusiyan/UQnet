...
class UQ_Net_std_TF2(Model):
    ...
    def call(self, x):
        ...
        x = tf.math.sqrt(tf.math.square(x) + 0.2)
        # x = tf.nn.relu(x) + 0.5
...
class CL_UQ_Net_train_steps:
    ...
    @tf.function
    def train_step_UQ_Net_mean_TF2(self, xtrain, ytrain, xTest, yTest):
    ...
    @tf.function
    def train_step_UQ_Net_std_UP_TF2(self, xtrain, ytrain, xTest, yTest):
        with tf.GradientTape() as tape:
    ...
    @tf.function
    def train_step_UQ_Net_std_DOWN_TF2(self, xtrain, ytrain, xTest, yTest):
        with tf.GradientTape() as tape:
